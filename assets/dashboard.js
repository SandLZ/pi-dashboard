...
$(document).ready(function () {
  ...
  var chartRAM = Highcharts.chart('container-mem',
    Highcharts.merge(gaugeOptions, {
      yAxis: {
        min: 0,
  ...
  var chartCache = Highcharts.chart('container-cache',
    Highcharts.merge(gaugeOptions, {
      yAxis: {
        min: 0,
        max: init_vals.mem.total,
  ...
  var chartRAM_real = Highcharts.chart('container-mem-real',
    Highcharts.merge(gaugeOptions, {
      yAxis: {
        min: 0,
        max: init_vals.mem.total,
  ...
  var chartSWAP = Highcharts.chart('container-swap',
    Highcharts.merge(gaugeOptions, {
      yAxis: {
        min: 0,
  ...
  setInterval(function () {
    ...
    $.getJSON('?ajax=true', function (data) {
      ...
      for (i = 0; i < data.net.count; i++) {
        $('#net-interface-' + (i + 1) + '-total-in').
          text(bytesRound(parseInt(data.net.interfaces[i].total_in), 2));
        $('#net-interface-' + (i + 1) + '-total-out').
          text(bytesRound(parseInt(data.net.interfaces[i].total_out), 2));
      }
    ...
    if (window.dashboard != null) {
      ...
      if (window.dashboard_old != null) {
          ...
          for (i = 0; i < window.dashboard_old.net.count; i++) {
            if (chartNetInterfaces[i].series[0].data.length >= 30) {
              chartNetInterfaces[i].series[0].addPoint(
                ...
                parseInt(window.dashboard_old.net.interfaces[i].total_in), true,
                true);
            }
            else {
              chartNetInterfaces[i].series[0].addPoint(
        ...
        idle_diff = parseInt(window.dashboard.cpu.stat.idle) -
        ...
        if (chartCPU) {
          ...
          point.update(Math.round(
              (1.0 - (idle_diff / used_total)) * 100 * Math.pow(10, 1)) /
            Math.pow(10, 1));
        }

        $('#cpu-stat-idl').
          text(Math.round(((parseInt(window.dashboard.cpu.stat.idle) -
              parseInt(window.dashboard_old.cpu.stat.idle)) / used_total) *
              100 * Math.pow(10, 1)) / Math.pow(10, 1));
        $('#cpu-stat-use').
          text(Math.round(((parseInt(window.dashboard.cpu.stat.user) -
              parseInt(window.dashboard_old.cpu.stat.user)) / used_total) *
              100 * Math.pow(10, 1)) / Math.pow(10, 1));
        $('#cpu-stat-sys').
          text(Math.round(((parseInt(window.dashboard.cpu.stat.sys) -
              parseInt(window.dashboard_old.cpu.stat.sys)) / used_total) * 100 *
              Math.pow(10, 1)) / Math.pow(10, 1));
        $('#cpu-stat-nic').
          text(Math.round(((parseInt(window.dashboard.cpu.stat.nice) -
              parseInt(window.dashboard_old.cpu.stat.nice)) / used_total) *
              100 * Math.pow(10, 1)) / Math.pow(10, 1));
        $('#cpu-stat-iow').
          text(Math.round(((parseInt(window.dashboard.cpu.stat.iowait) -
              parseInt(window.dashboard_old.cpu.stat.iowait)) / used_total) *
              100 * Math.pow(10, 1)) / Math.pow(10, 1));
        $('#cpu-stat-irq').
          text(Math.round(((parseInt(window.dashboard.cpu.stat.irq) -
              parseInt(window.dashboard_old.cpu.stat.irq)) / used_total) * 100 *
              Math.pow(10, 1)) / Math.pow(10, 1));
        $('#cpu-stat-sirq').
          text(Math.round(((parseInt(window.dashboard.cpu.stat.softirq) -
              parseInt(window.dashboard_old.cpu.stat.softirq)) / used_total) *
              100 * Math.pow(10, 1)) / Math.pow(10, 1));
      }
    }
...
function bytesRound(number, round) {
  if (number < 0) {
    ...
    number = number / 1024;
    var last = Math.round(number * Math.pow(10, round)) / Math.pow(10, round) +
      'K';
